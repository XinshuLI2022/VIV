...
def get_FLAGS():
    ...
    tf.app.flags.DEFINE_integer('epochs', 2, 'epochs ')
...
class VIV(object):
    ...
    def __init__(self, x_cont, x_disc, y, t, args, q_labels):
        self.x_cont = x_cont
        self.x_disc = x_disc
        self.x_one_hot = tf.one_hot(indices=(self.x_disc), depth=7, axis=(-1))
    ...
    def build_graph(self, args):
        ...
        logits_x_disc = self.fc_net(hx, [args.h], [[7, None]], 'px_ca_bin', lamba=(args.lamba), activation=activation)
        x_cont_hat = self.fc_net(hx, [args.h], [[1, None]], 'px_ca_cont', lamba=(args.lamba), activation=activation)
...
def generate_IV(data, resultDir, exp, input_args, iv_dir):
    ...
    with tf.Graph().as_default():
        ...
        x_ph_disc = tf.placeholder((tf.int32), [M], name='x_cate')
        x_ph_cont = tf.placeholder((tf.float32), [M, 1], name='x_cont')
        ...
        model = VIV(x_ph_cont, x_ph_disc, y_ph, t_ph, args, q_labels)
...
def trainNet(model, sess, train, val, test, args, logfile, modelfile, exp):
    ...
    dict_train = {model.x_cont: train['x'][:, 0:1], model.x_disc: train['x'][:, 1], model.y: train['y'], model.t: train['t'],model.q_labels: 0}
    dict_valid = {model.x_cont: val['x'][:, 0:1], model.x_disc: val['x'][:, 1], model.y: val['y'], model.t: val['t'], model.q_labels: 0}
    dict_test = {model.x_cont: test['x'][:, 0:1], model.x_disc: test['x'][:, 1], model.y: test['y'], model.t: test['t'], model.q_labels: 0}
    ...
    for epoch in range(n_epoch):
        ...
        for j in range(n_iter_per_epoch):
            ...
            dict_batch = {model.x_cont: x_batch[:, 0:1], model.x_disc: x_batch[:, 1], model.y: y_batch, model.t: t_batch, model.q_labels: 0}
            if not objnan:
