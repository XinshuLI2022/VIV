...
def run(exp, data, train_dict, log, device, resultDir, others):
    ...
    est_treat = feed_forward_net(treatment_input, lambda x: mixture_of_gaussian_output(x, train_dict['n_components']),
    ...
    treatment_model.fit([data.train.z, data.train.x], data.train.t, epochs=epochs, batch_size=train_dict['batch_size'])
    ...
    response_model.fit([data.train.z, data.train.x], data.train.y, epochs=epochs, verbose=1,
                        batch_size=train_dict['batch_size'], samples_per_batch=train_dict['samples_per_batch'])

    def estimation(data):
        return response_model.predict([data.x, data.t-data.t]), response_model.predict([data.x, data.t]),response_model.predict([data.x, data.e])
