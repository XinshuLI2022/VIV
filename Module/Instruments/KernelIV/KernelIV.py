...
def run(exp, data, train_dict, log, device, resultDir, others):
    ...
    trainer = KernelIVTrainer(data_list, train_config, use_gpu, one_dump_dir)
    train_loss, test_loss, mdl = trainer.train(rand_seed=42, verbose=verbose)

    def estimation(data):
        return mdl.predict(data.t-data.t, data.x), mdl.predict(data.t, data.x), mdl.predict(data.e, data.x)
